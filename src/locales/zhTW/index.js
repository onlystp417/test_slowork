export default {
  news: [
    '2022年12月22日（中央社）聯合國安全理事會今天通過決議案，要求緬甸軍政府釋放民主領袖翁山蘇姬。這是安理會有史以來，首次針對這個動盪不安的東南亞國家局勢通過決議案。',
    '2022年12月20日（中央社）翁山蘇姬案下週結辯，緬甸軍政府法庭將做最終判決。'
  ],
  header: {
    title: '緬甸，最後一搏',
    about: '關於',
    read: '閱讀',
    campaigns: '相關活動',
    buy: '購買本書'
  },
  main: [
    '緬甸政變將在2023年2月1日進入第三年，據緬甸人權組織「政治犯援助協會」統計，自政變以來，軍方逮捕逾1.6萬人、殺害2500多人。截至今年8月，聯合國人道事務協調（OCHA）推估已有130萬人離家失所，更有高達780萬名學童失學。相較烏克蘭、香港的抗爭，緬甸的民主之爭顯得孤立無援，且軍政府打壓手段並未趨緩，但緬甸人仍跨越族群，持續團結對抗獨裁強權。',
    '台灣的慢工文化，與長期關注緬甸局勢的法國編劇斐德希克．德波米、關注香港民主運動的漫畫家柳廣成攜手合作，以圖像小說的方式，紀錄下緬甸政變二年間所發生的事，和其背後的歷史、政治脈絡。本企畫公益性提供中*、英、泰、緬四語的公眾閱讀，期透過簡單易懂又富藝術的圖文，讓緬甸的人權問題和民主運動獲得更廣泛的理解與關注。'
  ],
  folks: {
    script: {
      name: '腳本 | 斐德希克．德波米',
      des: [
        '1975年出生，成長於巴黎。2006年起參與Info Birmanie、Euro Burma Network等支持、促進緬甸民主化的機構。他出版了十多本關於緬甸局勢的書籍和漫畫，並在媒體上發表相關文章，也曾針對盧旺達圖西人的種族滅絕問題發表著作。',
        "漫畫作品有《Sur le fil : dix ans d'engagement pour la Birmanie》、《Birmanie, fragments d'une réalité》、《Full stop : le génocide des Tutsi du Rwanda》等。"
      ]
    },
    comic: {
      name: '漫畫 | 柳廣成',
      des: [
        '香港創作者。童年成長於日本京都，深受當地漫畫文化影響，返回香港定居後修讀於香港中文大學藝術系。出版及連載短篇漫畫十餘部，並在香港反送中運動時大量以漫畫發聲。習慣以鉛筆作畫，個人風格明確，藉由筆觸強調氛圍，探索以漫畫作為表達媒介時的可能性，並嘗試解構重組「漫畫」。看似樸實的筆觸裡，實則有強烈的情緒。',
        '漫畫作品有《被消失的香港》、《報導者事件簿001-留學黑工》、《北港香爐人人插》'
      ]
    },
    publishing: {
      name: '慢工文化',
      des: [
        '專注出版原創圖像小說，以「多元性」作為出版核心、以「對真實的探索」作為創作的出發點。集結東亞及東南亞各國創作者，尋找屬於亞洲的圖像敘事風格。'
      ]
    }
  },
  readingTitle: '閱讀《緬甸，最後一搏》',
  reading: [
    '#周而復始',
    '#一段半自由的插曲',
    '#起來',
    '#奶茶聯盟',
    '#天使',
    '#恐怖分子',
    '#邊境暴行',
    '#團結 (請支持紙本）',
    '#孤軍奮戰1  (請支持紙本）',
    '#孤軍奮戰2  (請支持紙本）',
    '#Everything will be ok  (請支持紙本）'
  ],
  buy: {
    btn: '購買本書',
    des: [
      '支持本計劃，請購買中英雙語紙本書，並協助將書籍廣傳，支持全球運送。',
      '2023/2/28前，透過慢工文化直接購買本書，將捐出一成收入予相關團體。'
    ]
  },
  podcast: {
    title: '收聽PODCAST',
    list: ['緬甸民主運動家懷恩寧專訪', '緬甸，最後一搏繪者柳廣成專訪']
  },
  campaigns: {
    title: '《緬甸，最後一搏》相關活動',
    list: [
      {
        title: '台北國際書展講座：〈戰爭下的日常〉',
        date: '日期：2023年2月1日',
        time: '時間：15:30-17:45',
        location: '地點：世貿一館主題館迷你沙龍',
        speaker: '講者：柳廣成、杜可可、劉致昕',
        intro:
          '簡介：亞洲反極權狂潮中，面臨強權進逼的台灣，與《緬甸，最後一搏》作者香港漫畫家柳廣成、台灣聲援緬甸聯盟負責人杜可可，和《烏克蘭不可能的戰爭》共同作者劉致昕，從緬甸、香港及烏克蘭自由之戰，看見從日常回應非常。'
      }
    ]
  },
  footer: {
    organizer: {
      title: '製作單位',
      value: '慢工文化事業有限公司'
    },
    support: {
      title: '創作支持',
      value: '文化部、台灣文化基金會'
    },
    thanks_list: {
      title: '特別感謝',
      value: 'SEA plateaus'
    },
    contact: '合作、採訪、進書、版權洽詢'
  }
};
